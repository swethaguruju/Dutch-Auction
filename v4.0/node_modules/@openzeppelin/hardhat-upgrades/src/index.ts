...
import { DeployAdminFunction, makeDeployProxyAdmin } from '@openzeppelin/hardhat-upgrades/src/deploy-proxy-admin';
import type { DeployContractFunction } from '@openzeppelin/hardhat-upgrades/src/deploy-contract';
import type { ProposeUpgradeFunction } from '@openzeppelin/hardhat-upgrades/src/platform/propose-upgrade';
import type { GetDefaultApprovalProcessFunction } from '@openzeppelin/hardhat-upgrades/src/platform/get-default-approval-process';
...
subtask(TASK_COMPILE_SOLIDITY, async (args: { force: boolean }, hre, runSuper) => {
  const { readValidations, ValidationsCacheOutdated, ValidationsCacheNotFound } = await import('@openzeppelin/hardhat-upgrades/src/utils/validations');
...
subtask(TASK_COMPILE_SOLIDITY_COMPILE, async (args: RunCompilerArgs, hre, runSuper) => {
  ...
  const { writeValidations } = await import('@openzeppelin/hardhat-upgrades/src/utils/validations');

  // TODO: patch input
  const { output, solcBuild } = await runSuper();

  const { isFullSolcOutput } = await import('@openzeppelin/hardhat-upgrades/src/utils/is-full-solc-output');
...
if (tryRequire('@nomiclabs/hardhat-etherscan')) {
  subtask('verify:verify').setAction(async (args, hre, runSuper) => {
    const { verify } = await import('@openzeppelin/hardhat-upgrades/src/verify-proxy');
